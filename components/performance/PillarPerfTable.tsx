...
interface PillarPerfTableProps {
  groups: Group[];
}

export function PillarPerfTable({ groups }: PillarPerfTableProps) {
  if (groups.length === 0) return null;
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Hiệu suất theo pillar</CardTitle>
      </CardHeader>
      <CardContent>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left text-xs text-muted-foreground">
              <th className="py-2 pr-2 font-medium">Pillar</th>
